...
NOTE: this text originally posted on the [Haystack Blog](https://haystack.deepset.ai/blog/improve-retrieval-with-auto-merging){:target="_blank"} - I'm adding it to my personal blog for more awareness.
...
We will use the BBC news dataset to show how the `AutoMergingRetriever` works with a dataset containing multiple news articles. This dataset consists of 2.225 documents from the [BBC](http://news.bbc.co.uk/){:target="_blank"} corresponding to stories in five topical areas collected between 2004-2005, and was part of work by D. Greene and P. Cunningham. ["Practical Solutions to the Problem of Diagonal Dominance in Kernel Document Clustering", Proc. ICML 2006](http://mlg.ucd.ie/files/publications/greene06icml.pdf){:target="_blank"}.
